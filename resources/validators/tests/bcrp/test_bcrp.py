...
BCRP_VALIDATOR = Graph().parse(Path(__file__).parent.parent.resolve().parent / "items/bcrp.ttl")
VOCPUB_X_VALIDATOR = Graph().parse(Path(__file__).parent.parent.resolve().parent / "items/vocpub-x.ttl") + BCRP_VALIDATOR

VALIDATORS = {
    "bcrp": BCRP_VALIDATOR,
    "vocpub": VOCPUB_X_VALIDATOR,
}
...
def test_bcpr():
    ...
    for f in sorted(list(DATA_DIR.glob("*.ttl"))):
        ...
        validator_graph = None
        for k in VALIDATORS.keys():
            if k in str(f.name):
                validator_graph = VALIDATORS[k]
        if validator_graph is None:
            validator_graph = VALIDATORS["bcrp"]

        allow_warnings = False if "warnings" in str(f.name) else True

        data_graph = Graph().parse(f)

        v = validate(data_graph, shacl_graph=validator_graph, allow_warnings=allow_warnings, advanced=True, inference="rdfs")
